...
import { Plus, Package, Loader2, Search, Trash2, Edit3, X, Clock, AlertCircle, Camera, Image as ImageIcon } from 'lucide-react'
...
export default function FarmerProductsPage() {
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-chalk">
      <Loader2 className="animate-spin text-forest" size={40} />
      <p className="mt-4 text-stone/60 text-xs font-bold uppercase tracking-widest">Memuat Katalog...</p>
    </div>
  )
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-12 font-raleway pb-32 min-h-screen">
      <header className="mb-10 space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-forest leading-tight">Katalog Panen</h1>
            <p className="text-stone text-sm mt-1 font-medium">Kelola hasil bumi dan dokumentasi produk Anda.</p>
          </div>
          <Button 
            onClick={openAddModal} 
            className="bg-forest hover:bg-forest/90 text-chalk rounded-2xl h-12 w-12 md:w-auto md:px-6 shadow-xl shadow-forest/20 transition-all active:scale-95"
          >
            <Plus size={20} className="md:mr-2" />
            <span className="hidden md:inline font-bold">Tambah Produk</span>
          </Button>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 h-12 rounded-2xl bg-white border border-clay focus:ring-4 focus:ring-forest/5 outline-none text-sm transition-all" 
            />
          </div>
      ...
      {filteredProducts.length === 0 ? (
        <div className="text-center py-24 bg-white/50 rounded-[3rem] border-2 border-dashed border-clay">
          <Package size={48} className="mx-auto text-clay mb-4" />
          <p className="text-stone font-medium">Katalog masih kosong atau produk tidak ditemukan.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProducts.map((p) => (
            <Card key={p.id} className="group border-none shadow-sm hover:shadow-xl transition-all duration-500 bg-white rounded-[2.5rem] overflow-hidden">
              <div className="relative h-48 bg-chalk overflow-hidden group-hover:scale-105 transition-transform duration-700">
                {p.image_url ? (
                  <img src={p.image_url} alt={p.name} className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full flex flex-col items-center justify-center text-clay/40">
                    <ImageIcon size={40} strokeWidth={1} />
                    <p className="text-[10px] font-bold uppercase mt-2">Belum ada foto</p>
                  </div>
                )}
              ...
              <CardHeader className="pb-2 space-y-4">
                ...
                <CardTitle className="text-xl font-bold text-forest tracking-tight">
                  {p.name}
                </CardTitle>
              </CardHeader>
              ...
              <CardContent className="pt-2 space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div className={`p-5 rounded-3xl border shadow-inner transition-colors ${p.stock_kg < 10 ? 'bg-red-50 border-red-100' : 'bg-chalk border-clay'}`}>
                    <p className="text-[10px] font-bold text-stone uppercase tracking-widest mb-1">Stok</p>
                  ...
                  <div className="bg-chalk p-5 rounded-3xl border border-clay shadow-inner">
                    <p className="text-[10px] font-bold text-stone uppercase tracking-widest mb-1">Harga /kg</p>
                    <p className="text-lg font-bold text-forest tracking-tight italic font-mono">
                ...
                <div className="flex items-center gap-2 text-[10px] text-stone font-bold pt-2 uppercase tracking-tighter border-t border-chalk">
                  ...
                  Update: {new Date(p.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                </div>
              </CardContent>
            </Card>
          ))}
