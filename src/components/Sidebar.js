...
export default function Sidebar({ logout }) {
  ...
  return (
    <>     
      <aside className={`hidden md:flex flex-col bg-white shadow-xl border-r border-clay/30 h-screen sticky top-0 pt-4 transition-all duration-300 ease-in-out z-[100] ${isCollapsed ? 'w-24' : 'w-64'}`}>
        <div className="p-4 flex flex-col h-full relative">
          ...
          <div className={`flex items-center mb-10 transition-all duration-300 ${isCollapsed ? 'justify-center w-full' : 'px-4 gap-3'}`}>
            ...
            {!isCollapsed && (
              <span className="font-bold text-forest tracking-tighter text-2xl">Harsa</span>
            )}
          </div>
          ...
          <nav className="flex-1 space-y-2 w-full flex flex-col items-center font-raleway"> 
            ...
            <button 
              ...
              className={`flex items-center transition-all duration-300 rounded-2xl font-bold text-stone/60 hover:bg-chalk hover:text-forest ${isCollapsed ? 'justify-center w-14 h-14 p-0' : 'w-full px-4 py-3.5 gap-4'}`}
            >
              <HelpCircle size={22}/>
          ...
          <div className="pt-4 border-t border-clay mt-auto w-full">
            <button 
              onClick={logout} 
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-clay/10 z-[120] shadow-[0_-10px_25px_rgba(0,0,0,0.05)] pb-safe h-20">
          <div className="flex items-center justify-around px-2 h-full">
            <MobileNavLink 
              item={mainItems[0]} 
              isActive={pathname === mainItems[0].path && !isMenuOpen} 
            />
            
            <MobileNavLink 
              item={mainItems[1]} 
              isActive={pathname === mainItems[1].path && !isMenuOpen} 
            />
            ...
            <MobileNavLink 
              item={mainItems[2]} 
              isActive={pathname === mainItems[2].path && !isMenuOpen} 
            />
             
            <MobileNavLink 
              item={mainItems[3]} 
              isActive={pathname === mainItems[3].path && !isMenuOpen} 
            />
          </div>
      </nav>
        ...
        <div className="fixed inset-0 z-[110] flex flex-col justify-end">
          ...
          <div className="relative bg-white rounded-t-[3rem] p-6 pb-24 animate-in slide-in-from-bottom duration-500 shadow-[0_-20px_50px_rgba(0,0,0,0.1)] overflow-hidden font-raleway border-t border-clay/10">
            ...
            <div className="flex justify-between items-center mb-6 px-2">
              <h2 className="text-xl font-bold text-forest italic tracking-tight">Layanan harsa</h2>
            ...
            <div className="grid grid-cols-1 gap-3 overflow-y-auto max-h-[60vh] px-1 no-scrollbar">
              ...
              <div className="flex items-center gap-4 p-4 rounded-[1.5rem] bg-chalk border border-clay/20 active:scale-[0.98] transition-all min-h-[72px]">
                ...
                <div className="flex flex-col min-w-0">
                  <span className="text-xs font-bold text-stone truncate leading-tight">Notifikasi</span>
                  <span className="text-[10px] text-stone/50 truncate">Berita & update terbaru</span>
                </div>
              </div>
              ...
              >
                ...
                <div className="flex flex-col min-w-0">
                  <span className="text-xs font-bold text-forest truncate leading-tight">Pusat bantuan</span>
                  <span className="text-[10px] text-stone/50 truncate">Kontak & dukungan tim</span>
                </div>
              </button>
              
              <button 
                onClick={logout}
                className="mt-2 flex items-center justify-center gap-3 p-5 rounded-[1.5rem] bg-red-50 border border-red-100 active:scale-95 transition-all w-full"
              >
      ...
      {isHelpOpen && (
        <div className="fixed inset-0 z-[130] flex items-center justify-center p-6">
          ...
          <div className="relative bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in duration-300 font-raleway text-center">
            <div className="w-16 h-16 bg-chalk rounded-full flex items-center justify-center mx-auto mb-4 border-4 border-white shadow-lg">
              <MessageSquare size={28} className="text-forest" />
            </div>
            <h3 className="text-xl font-bold text-forest mb-2 tracking-tight">Pusat bantuan</h3>
            <p className="text-stone text-xs mb-6 leading-relaxed font-medium px-2">Punya kendala dengan transaksi atau akun Anda? Tim Harsa siap membantu Anda kapan saja.</p>
            
            <a 
              href="mailto:sarahfajriarahmah@gmail.com"
              className="flex items-center justify-center gap-3 w-full bg-forest text-white h-14 rounded-2xl font-bold shadow-xl shadow-forest/20 hover:bg-forest/90 transition active:scale-95 mb-4 text-sm"
            >
            ...
            <button 
              ...
              className="text-stone/40 text-[10px] font-bold uppercase tracking-widest hover:text-forest transition"
            >
              Tutup
            </button>
          </div>
...
function MobileMenuItem({ icon, label, path, onClick }) {
  return (
    <Link 
      href={path} 
      onClick={onClick}
      className="flex items-center gap-3 p-4 rounded-2xl bg-white border border-clay/30 active:scale-95 transition-all shadow-sm overflow-hidden"
    >
      <div className="p-2 bg-chalk rounded-xl text-forest shrink-0 flex items-center justify-center">
        {icon}
      </div>
