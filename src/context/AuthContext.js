...
export const AuthProvider = ({ children }) => {
  ...
  const authWithWallet = async (address) => {
    ...
    const { data: signUpData, error: signUpError } = await supabase.auth.signUp({
      email: dummyEmail,
      password: walletPassword,
      options: { data: { full_name: `User ${cleanAddress.slice(0, 6)}` } }
    })
    ...
    if (signUpData.user) {
      await supabase
        ...
        .update({ 
          ...
          full_name: `User ${cleanAddress.slice(0, 6)}`
        })
  ...
  return (
    <SupabaseContext.Provider value={{ 
      ...
      supabase, isInitialLoad, refreshProfile: () => user && fetchProfile(user.id) 
    }}>
      {children}
